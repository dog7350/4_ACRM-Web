    ...
    <div id="boardWriteRootWrapper" class="container-fluid d-flex align-items-center justify-content-center py-3 test-border border-radius-d fsps" >
        <div id="divUnderRoot" >
            <div class="container-fluid row d-flex align-items-center m-auto p-auto text-align-center" v-if="true">
                ...
                <div class="col-sm-12 my-1">
                    <textarea class="form-control awesome-scroll" id="content" placeholder="내용을 입력해주세요." style="height:10em; resize: none;" v-model="params.content"></textarea>
                </div>
                <div class="col-sm-12 my-1">
                    기존 사진
                </div>
                <div class="d-flex justify-content-around">
                    <div id="content" v-for="imgPath in params.currentImgPath" :key="imgPath.id">
...
export default {
    ...
    setup(props, context) {
        ...
        params.value.index = store.state.modifyForm.index;

        // console.log(store.state.modifyForm);
        ...
        const methods = {
            ...
            printFormData: ()=>{
                if(!params.value.isWrtings) {
                    ...
                    formData.append("boardIndex", params.value.index);
                    ...
                    AXIOS.put('/community/board', formData, {headers:{"Content-Type": "multipart/form-data"}})
                    .then((response)=>{
                        console.log(response.data);
                        store.commit('CREATE_ALERT', {msg:'성공적으로 수정되었습니다.', time: 2, type:"success"});
            ...
            loadDataForModify: ()=>{
                AXIOS.get(`/community/boards?bindex=${params.value.index+1}&pagesize=1`)
                .then((response)=>{
                    if(response.data.result && response.data.result.length > 0){
                        params.value.hideLevel = response.data.result[0].hideLevel;
                    }
                })
                .catch((error)=>{
                    store.commit('CREATE_ALERT', {msg:error.response.data.result, time: 2, type:"danger"});
        ...
        onMounted(()=>{
            ...
            params.value.title = store.state.modifyForm.title;
            params.value.content = store.state.modifyForm.content;
            params.value.contentType = store.state.modifyForm.type;
            params.value.hideLevel = store.state.modifyForm.hideLevel;
            params.value.currentImgPath = store.state.modifyForm.imgPath;
            // methods.loadDataForModify();
        });
